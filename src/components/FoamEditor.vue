...
const getImageUrl = (name) => {
  return new URL(`/src/assets/foam/${name}.png`, import.meta.url).href;
};

const sections = ref([
  { name: t('foamSection.clean'), path: getImageUrl('clean') },
  { name: t('foamSection.one'), path: getImageUrl('one') },
  { name: t('foamSection.two'), path: getImageUrl('two') },
  { name: t('foamSection.corner'), path: getImageUrl('corner') },
  { name: t('foamSection.three'), path: getImageUrl('three') },
  { name: t('foamSection.close'), path: getImageUrl('close') },
]);
...
const rotateSection = (index) => {
  if (circuit.value[index]) {
    circuit.value[index].rotation = (circuit.value[index].rotation + 90) % 360;
  }
};
...
const exportToPNG = async () => {
  ...
  html2canvas(gridContainer, {
    ...
    link.download = 'foam-grid-design.png';
...
const copyUrl = () => {
  navigator.clipboard.writeText(window.location.href)
    .then(() => {
      alert(t('foamEditor.urlCopied'));
    })
    .catch(err => {
      alert(t('foamEditor.urlCopiedError'));
    });
};
...
onMounted(() => {
  const cols = route.query.cols ? parseInt(route.query.cols, 10) : 10;
  const rows = route.query.rows ? parseInt(route.query.rows, 10) : 6;
  numCols.value = cols;
  numRows.value = rows;
  ...
  if (inventoryData) {
    try {
      ...
      console.error(t('foamEditor.loadInventoryError'), e);
    }
  }
  ...
  if (circuitData) {
    try {
      ...
      console.error(t('foamEditor.loadCircuitError'), e);
      router.push({ query: {} });
    }
  }
});
...
<template>
  <div class="flex h-screen">
    <div class="w-1/4 pt-4 pr-4 overflow-y-auto">
      <h2 class="text-lg font-bold mb-2">{{ $t('foamEditor.sections') }}</h2>
        ...
        >
          ...
          <div class="flex items-center justify-center mt-2 text-xs">
            <div class="flex items-center">
              <span class="mr-1">{{ $t('foamEditor.total') }}</span>
              <input type="number" min="0" v-model.number="inventory[section.name]" @click.stop class="w-11 p-1 border rounded text-sm">
            </div>
            <div>
              <span class="font-bold pl-2">{{ $t('foamEditor.availableSections') }} {{ availableSections[section.name] }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mt-8 p-4 border rounded bg-gray-50">
        <h3 class="text-md font-bold mb-2">{{ $t('foamEditor.controlsSections') }}</h3>
        <ul class="list-disc list-inside text-sm space-y-1">
          <li><strong>{{ $t('foamEditor.dragDropSections') }}</strong> {{ $t('foamEditor.dragDropSectionsAction') }}</li>
          <li><strong>{{ $t('foamEditor.rightClickSections') }}</strong> {{ $t('foamEditor.rightClickSectionsAction') }}</li>
          <li><strong>{{ $t('foamEditor.doubleClickSections') }}</strong> {{ $t('foamEditor.doubleClickSectionsAction') }}</li>
        </ul>
      </div>
    ...
    >
      <div class="flex justify-between items-center mb-4 shrink-0">
        ...
        <div class="flex items-center gap-4">
          ...
          <button @click="copyUrl" class="p-2 border rounded bg-blue-500 text-white text-sm">{{ $t('foamEditor.copyUrlButton') }}</button>
          <button @click="exportToPNG" class="p-2 border rounded bg-green-500 text-white text-sm">{{ $t('foamEditor.exportToPngButton') }}</button>
        </div>
      </div>
      <div class="flex-grow overflow-auto border rounded p-4">
        <div 
          ...
          :style="{ 
            'grid-template-columns': `repeat(${numCols}, 80px)`,
            'grid-template-rows': `repeat(${numRows}, 80px)`
          }"
        >
          <div
